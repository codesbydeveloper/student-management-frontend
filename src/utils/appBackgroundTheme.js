const STORAGE_KEY = 'sm_app_background_v1'
...
export function getAppBackgroundSnapshot() {
  if (typeof window === 'undefined') {
    return {
      sidebar: { ...DEFAULT_APP_BACKGROUND.sidebar },
      main: { ...DEFAULT_APP_BACKGROUND.main },
    }
  }
  try {
    ...
    return normalizeAppBackgroundTheme(JSON.parse(raw))
  } catch {
    return {
      sidebar: { ...DEFAULT_APP_BACKGROUND.sidebar },
      main: { ...DEFAULT_APP_BACKGROUND.main },
...
export function cacheAppBackgroundTheme(theme) {
  const next = normalizeAppBackgroundTheme(theme)
  if (typeof window !== 'undefined') {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(next))
    window.dispatchEvent(new CustomEvent(EVENT, { detail: next }))
  }
  return cloneTheme(next)
}
...
export function resetAppBackgroundTheme() {
  ...
  return cloneTheme(DEFAULT_APP_BACKGROUND)
}
