      ...
      <div className="space-y-4">
        ...
        {row.meetingNote ? (
          <div>
            ...
            <p className="mt-1 text-sm text-slate-800">{row.meetingNote}</p>
          </div>
        ) : null}
        {row.staffReviewNote ? (
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">School note</p>
            <p className="mt-1 text-sm text-slate-800">{row.staffReviewNote}</p>
          </div>
        ) : null}
        ...
        {row.status === PTM_STATUS.REJECTED && row.rejectionNote ? (
          <div>
            ...
            <p className="mt-1 text-sm text-red-800/90">{row.rejectionNote}</p>
          </div>
        ) : null}
