...
export function SidebarMenuAppearancePreview({ colors, items }) {
  ...
  const renderIcon = (key, { isGroup = false } = {}) => {
    ...
    if (item?.customIconUrl) {
      return (
        <span className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-white/10 ring-1 ring-white/20">
          <img src={item.customIconUrl} alt="" className="h-5 w-5 object-contain" decoding="async" />
        </span>
      )
    }
    if (item?.iconPreset) {
      return <NavIconTile navKey={item.iconPreset} isActive={activeKey === key} size="sm" />
    }
    if (isGroup) {
      return <NavIconTile groupKey={key} isActive={activeKey === key} size="sm" />
    }
    return <NavIconTile navKey={key} isActive={activeKey === key} size="sm" />
  }
